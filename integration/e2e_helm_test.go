...
func TestE2E_HelmDeployment(t *testing.T) {
	...
	// Verify DaemonSet is ready
	t.Log("Verifying DaemonSet is ready...")
	dsReady := strings.TrimSpace(runCmd(t, 30*time.Second,
		"kubectl", "get", "daemonset", releaseName, "-n", "kube-system",
		"-o", "jsonpath={.status.numberReady}"))
	if dsReady != "1" {
		t.Fatalf("DaemonSet not ready: numberReady=%s", dsReady)
	}
...
`, ns)
	...
	t.Log("Waiting for test pod...")
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	for {
		select {
		case <-ctx.Done():
			podYaml, _ := runCmdWithInput(10*time.Second, "", "kubectl", "get", "pod", "test-helm-ca", "-n", ns, "-o", "yaml")
			t.Logf("Pod YAML:\n%s", podYaml)
			t.Fatal("timeout waiting for test pod")
		case <-time.After(3 * time.Second):
			phase, _ := runCmdWithInput(10*time.Second, "", "kubectl", "get", "pod", "test-helm-ca", "-n", ns, "-o", "jsonpath={.status.phase}")
