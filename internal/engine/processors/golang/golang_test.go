...
func TestDetectApplicableWhenGoExists(t *testing.T) {
	t.Parallel()
	rootfs := t.TempDir()
	testutil.WriteExecutableInRootfs(t, rootfs, "/usr/local/go/bin/go")
...
func TestDetectNotApplicableWhenGoMissing(t *testing.T) {
	t.Parallel()
